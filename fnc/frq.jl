...
h1 = histogram(ld, bins=50, normalize=true, title="LD", color=1, ylabel="Distribution of allele frequencies")
h2 = histogram(md, bins=50, normalize=true, title="MD", color=2, xlabel="Allele frequencies")
h3 = histogram(hd, bins=50, normalize=true, title="HD", color=3)
hst = plot(h1, h2, h3, layout=(1,3), dpi=300, size=(1380,480), left_margin=10px, bottom_margin=15px, leg=false)
